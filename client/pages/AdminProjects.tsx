      ...
      if (editingId) {
        ...
        try {
          (await import("@/lib/audit")).recordAudit({
            ...
            entity: "project",
            entityId: editingId,
            details: `Updated ${form.project_title || form.project_name}`,
          });
        } catch {}
      } else {
        ...
        try {
          (await import("@/lib/audit")).recordAudit({
            ...
            details: `Created ${form.project_title || form.project_name}`,
          });
        } catch {}
      }
      const list = await fetchProjects();
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, project_summary: e.target.value })
              }
            />
          </div>
          <div className="space-y-1">
            ...
            <textarea
              className="w-full rounded-md border bg-background px-3 py-2 min-h-24"
          ...
          <div className="space-y-2">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && techInput.trim()) {
                    ...
                    setForm({
                      ...form,
                      techStacks: [...form.techStacks, techInput.trim()],
                    });
                    setTechInput("");
                  }
                }}
            ...
            <div className="flex flex-wrap gap-2">
              {form.techStacks.map((t, i) => (
                <span
                  key={`${t}-${i}`}
                  className="text-xs rounded-full bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300 px-2 py-1 border border-indigo-200/60 dark:border-indigo-500/20 flex items-center gap-1"
                >
                  {t}
                  <button
                    type="button"
                    className="text-indigo-700/70 hover:text-indigo-700"
                    onClick={() =>
                      setForm({
                        ...form,
                        techStacks: form.techStacks.filter(
                          (x, idx) => idx !== i,
                        ),
                      })
                    }
              ...
              <button
                ...
                onClick={() =>
                  setForm({
                    ...
                    who_involved: [
                      ...form.who_involved,
                      { name: "", email: "", designation: "", linkedin: "" },
                    ],
                  })
                }
              >
                ...
                <div key={idx} className="rounded-md border p-3 space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      value={m.name}
                      onChange={(e) => {
                        const a = [...form.who_involved];
                        a[idx] = { ...m, name: e.target.value };
                        setForm({ ...form, who_involved: a });
                      }}
                    />
                    <input
                      className="rounded-md border bg-background px-3 py-2"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      onChange={(e) => {
                        const a = [...form.who_involved];
                        a[idx] = { ...m, designation: e.target.value };
                        setForm({ ...form, who_involved: a });
                      }}
                    />
                    <input
                      ...
                      onChange={(e) => {
                        const a = [...form.who_involved];
                        a[idx] = { ...m, linkedin: e.target.value };
                        setForm({ ...form, who_involved: a });
                      }}
                    />
                  </div>
                  <div className="flex justify-end">
                    <button
                      ...
                      onClick={() =>
                        setForm({
                          ...form,
                          who_involved: form.who_involved.filter(
                            (_, i) => i !== idx,
                          ),
                        })
                      }
      ...
      <div className="lg:col-span-2 rounded-xl border bg-card p-5">
        ...
        <div className="grid gap-4">
          {projects.map((p) => (
            <div
              key={p._id}
              className="rounded-md border p-4 flex items-start justify-between hover:bg-accent/30"
            >
                  ...
                  onClick={async () => {
                    ...
                    try {
                      (await import("@/lib/audit")).recordAudit({
                        ...
                        entity: "project",
                        entityId: p._id,
                        details: `Deleted ${p.project_title || p.project_name}`,
                      });
                    } catch {}
